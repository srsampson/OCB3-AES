...
((a) == 0) ? 0 : (sizeof(uint32_t) * 8 - __builtin_clz((uint32_t) (a)) - 1)
...
#define OCB_NONCEPARAM int nonce_length,
#define OCB_ADPARAM const uint8_t * __restrict associated_data, int associated_data_length,
...
void hash(const uint8_t * __restrict round_key, OCB_ADPARAM
  const uint8_t l[][16], const uint8_t * __restrict l_asterisk,
...
void ocb_encrypt(const uint8_t * __restrict key, const uint8_t * __restrict nonce, OCB_NONCEPARAM
  const uint8_t * __restrict message, int message_length, OCB_ADPARAM uint8_t * out) {
...
int ocb_decrypt(const uint8_t * __restrict key, const uint8_t * __restrict nonce, OCB_NONCEPARAM
  const uint8_t * __restrict encrypted, int encrypted_length, OCB_ADPARAM
  uint8_t * __restrict out) {
